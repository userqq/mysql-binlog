 ...
 */
final class ParallelEventsIterator implements IteratorAggregate
{
    ...
    {
        $pipeline = \Amp\Pipeline\Pipeline::fromIterable(function () {
            try {
                foreach ($this->connection as $buffer) {
                    if (null !== $header = $this->parse($buffer)) {
                        yield [$header, $buffer];
                        // var_dump($header);
                        // exit();
                    }
        ...
        $pipeline = $pipeline
            ...
            ->map(function (array $data) {
                [$header, $buffer] = $data;
                $execution = \Amp\Parallel\Worker\submit(new ParseTask(
                    $header,
                    $buffer,
                    $this->tableMaps,
                ));

                $value = $execution->await();
        ...
        /*
            ...
            ->filter(fn (?Event $event) => !is_null($event))
            ->getIterator()
            ->getIterator();
        } catch (\Throwable $t) {
            echo $t . PHP_EOL;
    ...
    private function parse(Buffer $buffer): ?Header
    {
